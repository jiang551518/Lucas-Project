...
- **会话用量与耗时**：Token 用量按会话独立累计，数据库按每条 assistant 回复保存本轮输入/输出用量。切换会话时显示该会话的累计值；流式输出期间只更新对应会话。新对话显示为零，删除当前会话并返回新对话页时清零。模型未提供 usage 时会估算并标注。回复过程显示运行状态和耗时，完成后保留本轮用时。
...
- 输入与输出 Token 分别累计在各自的会话中，不跨会话混算。
- 切换到已有会话时，根据该会话保存的 assistant 回复用量重新显示累计值；模型仍在输出时，实时增量归属到发起请求的会话，切换到其他会话不会污染当前显示。
- 点击“新对话”显示空会话用量（0）；删除当前会话后回到新对话页面，用量也会清零。删除其他会话不会改变当前会话的用量。
- 服务商没有返回用量数据时，界面会显示估算值标记。
